...
# -*- coding: utf-8 -*-
"""
App Test Modbus RTU - "kieu Modbus Poll"
------------------------------------------------------------
- Ket noi RS485/Modbus RTU qua cong COM (chi dung pyserial, tu cai dat
  giao thuc Modbus RTU FC3/FC6 + CRC16, khong phu thuoc thu vien pymodbus).
- Doc lien tuc (poll) danh sach thanh ghi theo "Register Map" cau hinh san
  (mac dinh theo cam bien nhiet do/do am ES35-SW cua EPCB).
...
APP_TITLE = "App Test Modbus RTU (EPCB) - Modbus Pull Style Logger"
APP_VERSION = "1.0"
...
class ModbusPollApp(tk.Tk):
    ...
    def _build_connection_bar(self):
        bar = ttk.LabelFrame(self, text="Ket noi Modbus RTU (RS485)")
        bar.pack(fill=tk.X, padx=8, pady=(8, 4))

        ttk.Label(bar, text="Cong (Port):").grid(row=0, column=0, padx=4, pady=6, sticky="e")
        ...
        ttk.Button(bar, text="Lam moi", command=self._refresh_ports, width=8).grid(row=0, column=2, padx=2)

        ttk.Label(bar, text="Baudrate:").grid(row=0, column=3, padx=(16, 4), sticky="e")
        self.baud_var = tk.IntVar(value=9600)
        baud_combo = ttk.Combobox(bar, textvariable=self.baud_var, width=9, state="readonly",
                                   values=BAUDRATE_LIST)
        baud_combo.grid(row=0, column=4, padx=4)

        ttk.Label(bar, text="Parity:").grid(row=0, column=5, padx=(16, 4), sticky="e")
        self.parity_var = tk.StringVar(value="N")
        ttk.Combobox(bar, textvariable=self.parity_var, width=4, state="readonly",
                     values=["N", "E", "O"]).grid(row=0, column=6, padx=4)

        ttk.Label(bar, text="Slave ID:").grid(row=0, column=7, padx=(16, 4), sticky="e")
        self.slave_id_var = tk.IntVar(value=1)
        ttk.Spinbox(bar, from_=1, to=247, textvariable=self.slave_id_var, width=6).grid(row=0, column=8, padx=4)

        ttk.Label(bar, text="Scan rate (ms):").grid(row=0, column=9, padx=(16, 4), sticky="e")
        self.scan_rate_var = tk.IntVar(value=1000)
        ttk.Spinbox(bar, from_=200, to=60000, increment=100, textvariable=self.scan_rate_var,
                    width=8).grid(row=0, column=10, padx=4)

        self.connect_btn = ttk.Button(bar, text="Connect", command=self._toggle_connect, style="Accent.TButton", width=12)
        self.connect_btn.grid(row=0, column=11, padx=(16, 4))

        self.auto_scan_btn = ttk.Button(bar, text="Auto Scan", command=self._toggle_auto_scan, width=10)
        self.auto_scan_btn.grid(row=0, column=12, padx=4)

        self.conn_indicator = tk.Canvas(bar, width=16, height=16, highlightthickness=0)
        self.conn_indicator.grid(row=0, column=13, padx=(12, 4))
    ...
    def _build_chart_tab(self, parent):
        if not HAS_MPL:
            ttk.Label(parent, text="Thu vien matplotlib chua duoc cai dat. Chay: pip install matplotlib",
    ...
    def _build_settings_tab(self, parent):
        ...
        for i, r in enumerate(writable):
            ttk.Label(frame, text="%s (reg %d):" % (r["name"], r["address"])).grid(
                row=i, column=0, sticky="e", padx=6, pady=6)
            var = tk.StringVar()
            entry = ttk.Entry(frame, textvariable=var, width=12)
            entry.grid(row=i, column=1, padx=6, pady=6)
            self.write_entries[r["name"]] = var
            ttk.Button(frame, text="Apply", command=lambda reg=r, v=var: self._write_register(reg, v)).grid(
    ...
    def _build_bottom_panel(self, parent):
        ...
        self.log_text = tk.Text(tab_log, height=10, font=("Consolas", 9), bg="#111", fg="#0f0")
        ...
        ttk.Button(tab_log, text="Xoa log", command=self._clear_log).pack(side=tk.LEFT, padx=8, anchor="n", pady=8)
    ...
    def _build_excel_tab(self, parent):
        ...
        ttk.Label(row0, text="File Excel:").grid(row=0, column=0, sticky="e", padx=4)
        entry = ttk.Entry(row0, textvariable=self.log_filepath_var, width=70)
        entry.grid(row=0, column=1, padx=4)
        ttk.Button(row0, text="Chon file...", command=self._choose_excel_file).grid(row=0, column=2, padx=4)

        row1 = ttk.Frame(parent)
        row1.pack(fill=tk.X, padx=10, pady=4)
        self.start_log_btn = ttk.Button(row1, text="Bat dau ghi log (Start Logging)",
                                         command=self._toggle_logging, style="Accent.TButton")
        self.start_log_btn.grid(row=0, column=0, padx=4)
        ttk.Button(row1, text="Export snapshot Excel", command=self._export_snapshot).grid(row=0, column=1, padx=4)

        self.log_status_var = tk.StringVar(value="Chua ghi log.")
        ttk.Label(row1, textvariable=self.log_status_var, foreground="#555").grid(row=0, column=2, padx=16)
        ...
        hint = ttk.Label(
            ...
            foreground="#777", wraplength=1000
        )
    ...
    def _start_logging(self):
        ...
        self.start_log_btn.configure(text="Dung ghi log (Stop Logging)")
        self.log_status_var.set("Dang ghi log vao: %s" % os.path.basename(filepath))

    def _stop_logging(self):
        self.logging_active = False
        self.start_log_btn.configure(text="Bat dau ghi log (Start Logging)")
        save_error = None

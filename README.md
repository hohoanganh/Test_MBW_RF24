...
- Lệnh `rf stat` hiển thị đầy đủ: `RF_LINK=UP|DOWN LAST_DEV_ID=<id> AGE_MS=<n>
  ...
  từng thiết bị**.
- **Dòng `RF_DEVS` trên status** (FW 0.3.0): `rf stat` in thêm 1 dòng
  `RF_DEVS: <id>:UP|DOWN(<age>s,<loss‰>) ...` — breakdown UP/DOWN + loss từng
  `dev_id` ngay trên status (không cần gõ `rf devices` riêng). App test dùng dòng
  này vẽ lưới 16 ô thiết bị. Event đổi trạng thái cũng in nổi bật kèm nhãn:
  `>>> RF LINK UP: dev_id=<n> (SLAVE) @ <n>s uptime` và
  `!!! CANH BAO - MAT SONG: dev_id=<n> (SLAVE) - <n>s khong nhan duoc gi !!!`.
- **Log sự kiện MẤT LINK lưu Flash** (FW 0.3.0): mỗi khi 1 `dev_id` chuyển DOWN,
  firmware ghi 1 bản ghi vào Flash kèm **ngày/giờ RTC + loss% lúc đó** — giữ qua
  mất nguồn để chẩn đoán "đêm qua con nào rớt, lúc mấy giờ". Đọc/xóa bằng
  `log` / `log clear`, hoặc xem bảng + xuất file trên app. Thiết kế tiết kiệm
  RAM (chip chỉ 10KB): **không dùng RAM tĩnh**, vị trí ghi quét on-demand; chỉ
  log chiều DOWN (không log UP/khôi phục) để đủ chỗ. Cần lắp pin CR1220 và đồng
  bộ giờ (`rtc set` hoặc nút trên app) để mốc thời gian đúng.
...
| `rtc` / `rtc set [dd/mm/yy] hh:mm:ss` | Đọc/đặt **ngày + giờ** RTC PCF85063 (pin CR1220 giữ qua mất nguồn) — làm mốc thời gian cho log sự kiện. Nhận cả `rtc set hh:mm:ss` (chỉ giờ) |
...
| `log` / `log clear` | Đọc / xóa **log sự kiện MẤT LINK** lưu Flash (FW 0.3.0): mỗi `dev_id` khi DOWN được ghi kèm **ngày/giờ RTC + loss% lúc đó**, giữ qua mất nguồn. Vùng riêng `0x010000` (2048 bản ghi 8-byte, vòng lại khi đầy), **không tốn RAM tĩnh** (quét vị trí ghi on-demand). Định dạng dòng: `LOG: <#> dd/mm hh:mm:ss dev=<id> DOWN loss=<n%>` |
...
1. **Chuẩn bị từng board:** nạp cùng một phiên bản firmware (hiện tại **0.3.0**)
   cho MỌI board — cấu trúc khung RF từ 0.2.0 trở đi có byte `hop`, không trộn
   với firmware cũ hơn; `net id <n>` cùng giá trị cho cả
...
- **Giám sát Forward** – tab mặc định: bảng log trực quan mọi khung đã relay
  ...
  (rf stat)" xem % mất và mức dự phòng cố định, nút "Reset % mất gói (rf reset)"
  bắt đầu cửa sổ đo mới — tự làm mới mỗi 3 giây khi đang kết nối.
  **Lưới 16 ô thiết bị** (FW 0.3.0): mỗi ô là 1 `dev_id` (1..16) hiển thị lớn kèm
  **% mất gói**, tô màu xanh (tốt <2%) / cam (khá 2–10%) / đỏ (kém >10% hoặc
  MẤT LINK) / xám (chưa nghe thấy) — liếc 1 cái biết con nào cần bật repeater.
  **Cấu hình board:** ô **Net ID** với nút "Đọc ID" + "Set & Lưu Flash"
  (gửi `net id`, lưu vĩnh viễn); hàng **Đồng hồ RTC** với "⏱ Đồng bộ giờ máy tính"
  (gửi ngày+giờ PC xuống board) + "Đọc giờ". **Log sự kiện:** nút "📋 Đọc log
  sự kiện (Flash)" mở bảng lịch sử MẤT LINK (Ngày, Giờ, dev_id, Sự kiện, Loss)
  kèm nút **"💾 Lưu ra file (.log/.csv)"**, và nút "Xóa log". Nút **"🔌 Modbus Poll Test
...
### "Không forward được RS485" (RF_RX_OK=0, byte FWD ra rác) — 2026-07-06

Triệu chứng: `rf stat` báo `RF_LINK=UP`, `HB_RX` tăng đều, `RF_RX_CRCERR=0`
(→ **RF hoàn toàn khỏe**), nhưng `RF_RX_OK=0` / `RF_TO_RS485=0` và các dòng
`FWD RS485->RF` in ra **byte rác** (vd `8C 05 D4 FF` hoặc `06 1E 00 ...`, độ dài
bất thường so với frame Modbus 7-8 byte).

Nguyên nhân đã gặp: **baud giữa Master ↔ board KHÔNG khớp** — KHÔNG phải lỗi
firmware / DIP / parity / RF. Dấu vân tay lệch baud (mô phỏng UART tái tạo khít
từng byte):
- byte bị **rút ngắn** (~½ số byte, hay bắt đầu bằng `8C`) → board đọc CHẬM hơn
  dây (dây nhanh hơn baud board — vd module Serial-over-Bluetooth đặt UART 19200
  dù ô Baudrate trong app ghi 9600);
- byte bị **nhân đôi** (~2× số byte, hay bắt đầu bằng `06 1E`) → board đọc NHANH
  gấp đôi (Master đặt 4800 còn board 9600).

Cách khoanh vùng: gõ `dip` để xác nhận BAUD firmware **thực sự** dùng; rồi
`bridge off` + `rs485mon on` để xem byte RS485 thô — baud nào cho ra `01 03 ...`
sạch (bắt đầu bằng slave ID) là baud thật của dây. **Đặt CẢ chuỗi cùng 1 baud**:
app Master + DIP hub + DIP slave + thiết bị Modbus thật. Số baud hiển thị trong
app Master (nhất là khi đi qua module Bluetooth-serial) có thể KHÁC baud UART
vật lý — kiểm tra cấu hình module, hoặc cắm thẳng USB-RS485 để loại biến.

### App bật lên rồi tắt ngay (crash lúc khởi động)

Kinh nghiệm 2026-07-06: `pady=(a, b)` dạng tuple chỉ hợp lệ trong `.pack()` /
`.grid()`, **không** hợp lệ khi truyền làm option của widget `tk.Label(...)` →
ném `TclError` lúc dựng UI. Nếu app flash rồi thoát, chạy `python mbw_test_app.py`
trong cmd để xem traceback.
